...
def _get_records_not_nan_mask(records_array):
...
def _generic_test__merge_many_records_group(test_case_file: str):
    ...
    print(pd.DataFrame(records, columns=DF_SCHEMA.keys()).astype(DF_SCHEMA).to_string())
    ...
    if isinstance(expected_output, str) and expected_output.endswith('.parquet'):
