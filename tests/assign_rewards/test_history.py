...
# from history import unique_hashed_history_file_name
# from history import select_incoming_history_files_for_node
# from history import history_files_for_hashed_history_id
# from history import histories_to_process
...
#
#
# def gen_random_hash_like_string():
...
#         hex_name = gen_random_hash_like_string()
...
#
#
# def test_hash_history_id__returns_hash():
...
#
#
# def test_history_dir_for_hashed_history_id__returns_correct_path():
...
#
#
# def test_unique_hashed_history_file_name__matches_pattern():
#     """ Test for the generated hashed history file name pattern """
#
#     hashed_history_id = gen_random_hash_like_string()
#
#     # Regular expression for the hashed history id
#     regexp1 = "[0-9a-f]{" + str(HASHED_HISTORY_ID_LEN) + "}"
#     # Regular expression for the uuid4
#     regexp2 = "[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}"
#     # Concatenated regular expression
#     regexp  = fr"{regexp1}-{regexp2}\.jsonl\.gz\Z"
#
#     unique_hashed_history_id = unique_hashed_history_file_name(hashed_history_id)
#     assert re.match(regexp, unique_hashed_history_id)
#
#
# def test_hashed_history_id_from_file__returns_history_id():
#     hashed_history_id = "edf73067d918a9df5c65d74e9e88305b5a6898a5c995189718e4d522bba8ebfb"
#     filename = Path(f"{hashed_history_id}-0fde9357-e87d-417f-88b4-0c8491dc7b76.jsonl.gz")
#     assert hashed_history_id_from_file(filename) == hashed_history_id
#
#
# def test_history_files_for_hashed_history_id(monkeypatch):
#     """
#     TODO:
#         - Add more known files
#         - Add known files that should not be present in the results
#
#     """
#
#     hashed_history_id = "653eb60f47df3af3bb8e2c751934314d1b281680b2ffa4f9278097e930f77548"
#
#     test_data_path = Path(os.environ['TEST_DATA_DIR']) / "assign_rewards"
#
#     # Patch config variable to make it point to a custom dir with known structure and files
#     monkeypatch.setattr(config, "HISTORIES_PATH", test_data_path)
#
#     # Assert that some known path files are found in a known folder
#     results = history_files_for_hashed_history_id(hashed_history_id)
#
#     known_files = [
#         f"{hashed_history_id}-c0f76ecf-7d81-4783-b2ea-7a2be8eb0bc6.jsonl.gz"
#     ]
#
#     assert len(results) == 1
#
#     for s in known_files:
#         fp = history_dir_for_hashed_history_id(hashed_history_id) / s
