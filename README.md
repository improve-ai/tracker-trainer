...
The Improve AI Gym is a scalable stack of serverless services that make it easy to deploy *reinforcement learning* systems in production. *Reinforcement learning* is a type of machine learning where models are trained simply by assigning rewards for positive outcomes. By aligning rewards with business metrics, such as revenue, conversions, or user retention, reinforcement learning automatically optimizes decisions to lift those metrics. In practice, tracking rewards is very similar to tracking analytics and can often be implemented in the same code.
