...
def worker():
    print(
        f'starting reward assignment job on node {config.NODE_ID} ({config.NODE_COUNT} nodes total)')
    ...
    grouped_incoming_history_files = utils.group_files_by_hashed_history_id(
        incoming_history_files)

    print(
        f'processing {len(grouped_incoming_history_files)} histories across {len(incoming_history_files)} incoming history files')
