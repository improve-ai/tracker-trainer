...
def worker():
    ...
    custom_print(f'Starting firehose ingest')
    ...
    custom_print(f"Made a total of {stats.s3_requests_count} requests to S3")
    custom_print(f'Finished firehose ingest')
...
def process_decisions(decision_partition: RewardedDecisionPartition):
    if SIGTERM:
        ...
        custom_print(f'Quitting due to SIGTERM signal')
